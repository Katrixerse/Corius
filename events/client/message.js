...
module.exports = async (bot, message) => {
  ...
  con.query(`SELECT gp.prefix, gs.modOnly, gds.commands, gs.levelingEnabled, gs.economyEnabled, gds.categories FROM guildPrefix AS gp LEFT JOIN guildDisabledSettings AS gds ON gp.guildId = gds.guildID LEFT JOIN guildSettings as gs ON gs.guildId = gds.guildId WHERE gp.guildId =${message.guild.id}`, async (e, row) => {
    con.query(`SELECT a.userId AS afkId, a.guildId, a.isAfk, a.afkReason FROM afk AS a WHERE a.guildId = '${message.guild.id}'`, async (e, rows) => {
      con.query(`SELECT * FROM guildModerators WHERE guildId = '${message.guild.id}'`, async (e, mods) => {
        con.query(`SELECT * FROM guildBlacklistedUsers WHERE guildBlacklistedUsers.guildId ="${message.guild.id}" AND user ="${message.author.id}"`, async (e, user) => {
          row = row[0];
